...
while True:
    ...
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)  # 그레이 이미지로 변환 - 명암의 차이를 가지고 검출하는거라서 그레이로 변환
    faces = face_cascade.detectMultiScale(gray, 1.3, 5) # 얼굴 검출,기본에서 1.3배 큰 얼굴 까지 검출,
    # face_cascade.detectMultiScale(이미지 변수, 스케일 요소, 얼굴 신뢰도)
    # 신뢰도 : 얼굴에 최소한 5개의 후보 경계 박스가 있어야 해당 얼굴을 검출

    ksize = 15  # 모자익 3-19 참조 - 필터(ksize) 크기 클 수록 더 많이 스무딩 됨
