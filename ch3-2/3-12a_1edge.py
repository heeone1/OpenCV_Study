...
#1차 미분 : Prewitt
prewitt_filter_x = np.array([[1, 0, -1], [1, 0, -1], [1, 0, -1]]) #수직에 해당하는 엣지 구하겠다 = 수직 엣지 필터
prewitt_filter_y = np.array([[-1, -1, -1], [0, 0, 0], [1, 1, 1]]) #수평에 해당하는 엣지 구하겠다 = 수평 엣지 필터
prewitt_grad_x = cv2.filter2D(blur, -1, prewitt_filter_x) # 수직 에지
prewitt_grad_y = cv2.filter2D(blur, -1, prewitt_filter_y) # 수평 에지
prewitt_x = cv2.convertScaleAbs(prewitt_grad_x)   # 절대값(Abs)을 취해 양수 영상으로 변환, 수직 에지
...
#2차 미분 : Sobel -> Prewitt은 함수 없는데 Sobel은 함수 있음
