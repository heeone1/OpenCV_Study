...
se1=np.uint8([[0,0,1,0,0],			# 구조 요소
            ...
            [0,1,1,1,0],
            [0,0,1,0,0]])
se2=cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5,5)) #getStructuringElement함수 사용하면 자동으로 위에 적힌 행렬 만들어줌
k=1     # 반복 횟수
...
# b_opening=cv2.morphologyEx(b,cv2.MORPH_OPEN, se2) #위에 코드와 동일한데 함수 사용해서 한거임, 침식 -> 팽창
b_closing=cv2.erode(cv2.dilate(b,se2,iterations=k),se2,iterations=k)	# 닫기
# b_closing=cv2.morphologyEx(b,cv2.MORPH_CLOSE, se2) #위에 코드와 동일, 팽창 -> 침식
