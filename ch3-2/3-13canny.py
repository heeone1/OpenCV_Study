...
blur=cv2.GaussianBlur(gray,(3,3), 1.0)	# Canny 1단계 : 가우시안 블러로 (전처리과정) 노이즈 제거
...
_, th = cv2.threshold(sobel_edge, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU) # Sobel 에지 결과에 임계값 구해서 이진화
